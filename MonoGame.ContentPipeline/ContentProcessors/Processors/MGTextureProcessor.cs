...
namespace MonoGameContentProcessors.Processors
{
    ...
    public class MGTextureProcessor : TextureProcessor
    {
        ...
        public override TextureContent Process(TextureContent input, ContentProcessorContext context)
        {
            ...
            {
                var height = face[0].Height;
                var width = face[0].Width;
                var mipLevels = 1;

                var needsResizing = height != width || !(isPowerOfTwo(height) && isPowerOfTwo(width)) || height > MAX_TEXTURE_SIZE || width > MAX_TEXTURE_SIZE;

                // TODO: Reflector ResizeToPowerOfTwo(TextureContent tex)
                // Resize the first face and let mips get generated from the dll.
                if (needsResizing)
                {
                    byte[] originalBytes = face[0].GetPixelData();
                    ...
                    context.Logger.LogWarning("", input.Identity, "Texture resized to: " + face[0].Width + "x" + face[0].Height, this);
                ...
                if (PremultiplyAlpha)
                {
                    var colorTex = face[0] as PixelBitmapContent<Microsoft.Xna.Framework.Color>;
                    if (colorTex != null)
                    {
                    ...
                    {
                        var vec4Tex = face[0] as PixelBitmapContent<Vector4>;
                        if (vec4Tex == null)
                ...
                ConvertToPVRTC(face, mipLevels, PremultiplyAlpha, compressionMode);
            }

            return input;
        }

        public static void ConvertToPVRTC(TextureContent input, int mipLevels, bool premultipliedAlpha, MGCompressionMode bpp)
        {
            foreach (MipmapChain face in input.Faces)
            {
                ConvertToPVRTC(face, mipLevels, premultipliedAlpha, bpp);
            }
        }

        public static void ConvertToPVRTC(MipmapChain face, int mipLevels, bool premultipliedAlpha, MGCompressionMode bpp)
        {
